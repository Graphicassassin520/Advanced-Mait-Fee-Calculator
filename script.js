...
function calculateProjection() {
    ...
    data.forEach((amount, index) => {
        tableHtml += `<tr><td>${index + 1}</td><td>${parseFloat(amount).toLocaleString()}</td><td>${parseFloat(data2[index]).toLocaleString()}</td><td>${parseFloat(savingsData[index]).toLocaleString()}</td></tr>`;
    });
    tableHtml += `<tr><td colspan="3">Total Savings over 20 Years:</td><td>${parseFloat(totalSavings).toLocaleString()}</td></tr></table>`;
